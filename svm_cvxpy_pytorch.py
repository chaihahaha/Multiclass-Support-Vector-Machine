...
class svm_model_cvxpy:
    ...
    def predict(self,x_np):
        ...
        k_predicts = (self.y_matrix.to(self.device) * self.a_matrix) @ self.kernel(xp,self.x).T  + self.b
    ...
    def wTx(self,k,xi):
        ...
        y = self.y_matrix[k, :].view(-1,1).to(self.device)
        a = torch.from_numpy(self.a[k].value).float().view(-1,1).to(self.device)
